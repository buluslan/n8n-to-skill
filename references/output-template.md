...
### name
- 全小写 + 连字符分词（`amazon-review-analyzer`，非 `AmazonReviewAnalyzer`）
...
- 常见坑：写成工具功能列表（"读 CSV、调 AI、写文件"）而非业务定位（"把评论变洞察"）

#### 2. 触发
- 列用户输入的形态：关键词、文件类型、命令模式
- 写成可执行判断："用户给 CSV 路径 + 含'分析/打标签/出洞察'任一关键词时启动"
...
- 常见坑：照抄原 workflow 的 credentials 清单（包含已被目标对等消化的 `googleSheetsOAuth2Api` 等）
...
## 五、案例：评论分析产物 skill 的 SKILL.md 段落填法

> 点到为止，完整骨架见 `assets/skill-template.md`。

### frontmatter 填法
- name：`amazon-review-analyzer`
- description 三段式：
  - 能力句："把一批亚马逊评论变成「标签分布 + 洞察报告」，给跨境卖家做产品迭代决策"
  - 触发词："当用户提供评论 CSV 路径，并说'分析评论 / 打标签 / 出洞察'时调用"
  - 反触发词："不负责实时爬取评论（评论须现成）；不处理非亚马逊来源"
- allowed-tools：`Read, Write, Edit`（生成式 AI 由 Claude 原生提供，无需 MCP）
- metadata：`source_workflow: review-analysis.json` / `goal_alignment: 评论→标签→统计→洞察全链路` / `conversion_date: 2026-07-20`（ISO 格式 YYYY-MM-DD）

### 8 段填法要点（每段一行示范）
- **定位**："读 CSV → AI 打标签 → 统计分布 → AI 生成洞察 → 输出 3 个文件"
- **触发**：CSV 路径 + 关键词集合（分析/标签/洞察/评论）
- **输入**：评论 CSV，必含 `review_id` + `text` 两列；可选 `rating` / `date`
- **输出**：`tagged.csv`（每条评论 + 标签）/ `stats.csv`（标签分布）/ `insight.md`（洞察报告）
- **核心能力**（4 条，每条一行）：读 CSV / AI 批量打标 / 标签统计 / AI 生成洞察
- **流程**：4 步顺序号 + 每步产物（① 校验 CSV → 打印行数；② 分批打标 → 进度日志；③ 统计 → 写 stats.csv；④ 生成洞察 → 写 insight.md）
- **凭证边界**：无（目标对等重写后，原 Gemini key 与 googleSheets OAuth 均已消化，详见 `credential-safety.md` 第六节）
- **失败降级**：CSV 缺列 → 报错并指明缺哪列；AI 单条失败 → 重试 3 次后跳过并记日志

### 关键收敛示范
原 n8n 18 节点 → 产物 SKILL.md 4 条核心能力 + 4 步流程。这是目标对等的最终呈现：节点结构不映射，业务目标对齐即可。

收敛过程不在此展开——节点 → 能力的映射逻辑见 `references/node-mapping.md`，凭证消化判定见 `references/credential-safety.md`。
