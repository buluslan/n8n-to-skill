...
| `executeOnce` | bool | ⚠️ | 仅在首轮执行。语义关键——评论分析 workflow 的"读取所有已打标数据"节点带此标记，表示循环结束后**只读一次**全量数据，不随循环重复 |
...
"循环处理评论": {
  "main": [
    [ {"node": "读取所有已打标数据", "type":"main","index":0} ],   // port=0 → done
    [ {"node": "Gemini评论打标",     "type":"main","index":0} ]    // port=1 → loop
  ]
},
"写回打标结果": {
  "main": [
    [ {"node": "循环处理评论", "type":"main","index":0} ]          // 回环到 splitInBatches
  ]
}
```

读出来的拓扑：每条评论走 `Gemini打标 → 解析 → 写回`，写回后**回到 `splitInBatches` 自身**触发下一批；全部批次结束后从 port=0 进入 `读取所有已打标数据`。这是一个典型的"循环体回环 + 完成分支"结构。
...
| `{{ $('节点名').item.json.field }}` | **跨节点引用**：取名为"节点名"的节点的输出（评论分析 workflow 的"准备写入数据"节点用 `{{ $('提取洞察内容').item.json.insightText }}` 取洞察文本） |
...
1. **能翻等价就翻**：表达式映射到的新 skill 实现里，用对应变量/函数替换。例：`{{ $json.name }}` → 新实现里的 `row["name"]`；`{{ $('提取洞察内容').item.json.insightText }}` → 上一步的 Python 变量。
...
> 本节拓扑图用于演示**怎么读 connections 结构**（workflow-anatomy 的职责）。从同一拓扑**提炼业务目标**的走法见 `goal-parity-design.md` §2.1，两处视角不同、不重复。

**18 节点 / 18 条边 / 1 触发器 / 1 循环 / 2 个生成式 AI / 8 个存储协作**。完整拓扑（按 connections 读出）：
...
**★ 标记的 4 个节点**（获取表格元数据 / 提取SheetId / 扩展列宽至30列 / 创建洞察报告表）业务贡献是**Sheets 表结构管理**——本地 CSV 方案无表结构概念，这些节点的目标贡献被"目标对等"消化掉（详见 `goal-parity-design.md`）。

### 6.1 读拓扑的标准动作（示范）

拿到一份 connections，按这 4 步读：

1. **找入口**：`entry_nodes` = `classify=="1_触发器"` 的节点。本例 = `表单触发`。
2. **顺主链**：从入口沿 `main[0][0].node` 走到底。本例：表单触发 → 获取表格元数据 → 提取SheetId → 扩展列宽至30列 → 初始化标签列头 → 读取评论数据 → 验证数据 → 循环处理评论。
3. **识别多端口节点**：遇到 `IF` / `Switch` / `splitInBatches`，按 §3.1 查端口语义。本例 `循环处理评论` 是 splitInBatches，port=0(done) 出循环、port=1(loop) 进循环体。
4. **识别回环**：找 `from==to` 的隐式回环（splitInBatches 的循环体末端回到自己）。本例 `写回打标结果 → 循环处理评论` 就是回环边。

读完拓扑，下一步去 `goal-parity-design.md` 设计等价实现（那里有从同一拓扑提炼的业务目标与收敛推演），而不是逐节点翻译。
